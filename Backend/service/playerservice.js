...
exports.playerregister=async(req,res)=>
{
  ...
  if (exist) {
    return res.status(400).json({ success: false, msg: "player already exist" });
  } else {
    ...
    if (data.length == 0) {
      ...
      return res
        .status(201)
        .json({ success: true, msg: "player registered successfully" });
    } else {
      let record = new rec({
        email: email,
        password: hp,
...
exports.playerlogin=async(req,res)=>
{
  ...
  if (pass) {
    ...
    return res
      .status(200)
      .json({ success: true, msg: "player login successfully",token:token});
  } else {
